...
Fix‑ISH AI is a calm, cinematic, master‑technician built to educate, repair, and protect. It evolves in skill and wisdom, never in ethics.`;
    ...
    async function callOpenAI({
      ...
      // Fallback: parse reasoning and confidence from raw text
      const reasoningMatch = /REASONING:\s*(.+?)(?=CONFIDENCE:|$)/is.exec(raw);
      const confMatch = /CONFIDENCE:\s*(0?\.\d{1,2}|1\.0)/i.exec(raw);
      
      const reasoning = reasoningMatch ? reasoningMatch[1].trim() : 'Response generated by Fix‑ISH AI';
      const conf = confMatch ? Math.min(1, Math.max(0, parseFloat(confMatch[1]))) : 0.85;
      
      // Remove reasoning/confidence metadata from visible response
      let cleanResponse = raw
        .replace(/REASONING:\s*.+?(?=CONFIDENCE:|$)/is, '')
        .replace(/CONFIDENCE:\s*0?\.\d{1,2}/i, '')
        .trim();
      
      return {
        response: cleanResponse || 'Hey there — what are we fixing today? I can analyze images and guide you step‑by‑step.',
        summary: reasoning,
        confidence: conf,
        status: 'success',
      };
